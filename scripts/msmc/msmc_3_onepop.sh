...
NR_IND=3
POP_OR_IND=`basename $1 .ind`
DATE=`date +%m%d%y`
RUN_NAME=msmc_${POP_OR_IND}_${DATE}_30seg
METHOD="samtools + mask"

find input/msmc_input.${POP_OR_IND}.ASB*.txt -size 0 -delete
#ls input/msmc_input.${POP_OR_IND}.ASB*.txt | grep -v 'LG9' > input/SCAFS_INPUT_${POP_OR_IND}_noLG9
ls input/msmc_input.${POP_OR_IND}.ASB*.txt > input/SCAFS_INPUT_${POP_OR_IND}
